...
import { getResidentDoctrine, getDoctrineOfDoctrines, buildResidentBootstrap } from './doctrine.js';
...
async function handle(req,res){
  ...
  if(req.method==='GET'&&url.pathname==='/health'){const live=getLiveState();const meta=getDoctrineOfDoctrines();return json(res,200,{status:'ok',service:'ghost-atlas-map-of-reality',version:VERSION,validation:validateModel(),doctrine:{version:getResidentDoctrine().version,status:'ACTIVE'},meta_doctrine:{version:meta.version,status:meta.status},physical_evidence:{state:live.state,age_seconds:live.age_seconds},organism_packet:{contract:'GA-C2P-1',state:buildOrganismPacket(live).state},time:new Date().toISOString()});}
  ...
  if(req.method==='GET'&&url.pathname==='/api/v1/omnitheon/doctrine-of-doctrines') return json(res,200,getDoctrineOfDoctrines());
  ...
  if(req.method==='GET'&&url.pathname==='/api/v1/proof'){
    ...
    return json(res,200,{state:live.state==='FRESH_PROVEN'?'IMPLEMENTATION_PLUS_FRESH_PHYSICAL_EVIDENCE':'IMPLEMENTATION_PROOF',physical_evidence:live,organism_packet:buildOrganismPacket(live),doctrine:{version:getResidentDoctrine().version,status:'ACTIVE'},meta_doctrine:{version:getDoctrineOfDoctrines().version,status:'CANONICAL'},truthBoundary:'Software invariants are local proof. Physical EDEN is promoted only from signed workflow evidence and expires into STALE; sensory packets and doctrines never self-promote authority.',validation:validateModel()});
  }
  if(req.method==='GET'&&url.pathname==='/api/v1/openapi.json') return json(res,200,{openapi:'3.1.0',info:{title:'Ghost Atlas Map of Reality API',version:VERSION},paths:{'/health':{get:{summary:'Health, invariants, doctrine stack and physical-evidence freshness'}},'/api/v1/map':{get:{summary:'Complete Map of Reality snapshot'}},'/api/v1/organs':{get:{summary:'Original 23-organ Vishvarupa registry'}},'/api/v1/wheel':{get:{summary:'Wheel of Reality control loop'}},'/api/v1/campaign':{get:{summary:'Convergence campaign state'}},'/api/v1/live':{get:{summary:'Latest sanitized physical-Estate evidence'}},'/api/v1/live/history':{get:{summary:'Ephemeral recent physical evidence history'}},'/api/v1/organism/packet':{get:{summary:'GA-C2P-1 sensory packet for Workforce Spine / Packet OS'}},'/api/v1/omnitheon/doctrine':{get:{summary:'Machine-readable OMNITHEON resident and workforce doctrine'}},'/api/v1/omnitheon/doctrine-of-doctrines':{get:{summary:'Estate-wide meta-doctrine governing doctrine precedence and conflict'}},'/api/v1/omnitheon/bootstrap':{get:{summary:'Resident startup doctrine-stack bootstrap contract'}},'/api/v1/ingest/physical':{post:{summary:'GitHub-OIDC authenticated physical EDEN census ingest'}}}});
